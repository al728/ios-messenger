...
// The view controller for the chat screen between two users.
...
class conversationVC: UIViewController, UIScrollViewDelegate, UITextViewDelegate {
    ...
    //Retreives messages between the two users from the "Message" Table
    func getMessageFunc(){
        refreshResults()
    }
    ...
    /**Changes the display of the view with newest messages shown on the botton
    * of the scroll view
    **/
    func refreshResults(){
        let theWidth = view.frame.size.width
            ...
            if error == nil {
                ...
                for var i = 0;  i <= self.messageArray.count-1; i++ {
                    ...
                    if self.senderArray[i] == userName{
                        ...
                        messageLbl.backgroundColor = UIColor.blackColor()
